...
/*
optimal solution without extra loop
#include <iostream>
#include <vector>
...
int main()
{
    ...
    while (t--)
    {
        ...
        // Step 1: Ensure sum >= 0
        // We need to flip some -1s to 1s if sum < 0
        if (sum < 0) {
            int flipsNeeded = (-sum + 1) / 2; // Ceiling division
            operations += flipsNeeded;
            negCount -= flipsNeeded;
            sum += 2 * flipsNeeded;
        }

        // Step 2: Ensure product = 1 (even number of -1s)
        if (negCount % 2 == 1) {
            operations++;
        }
